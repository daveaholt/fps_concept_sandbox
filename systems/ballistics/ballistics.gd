...
static func tracer_transform(head: Vector3, direction: Vector3, length: float,
		width: float = 1.0) -> Transform3D:
	...
	var thickness := maxf(width, 0.001)
	var basis := Basis(aligned.x * thickness, aligned.y * thickness,
		aligned.z * maxf(length, 0.01))
