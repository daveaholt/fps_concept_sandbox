...
- **Meshes that would block an occupant's view belong to the `vehicle_shell` group.** `VehicleCommon` moves that group to render layer 2 while the occupant's camera is inside the vehicle, and every camera in the project excludes layer 2 — the same mechanism the player already uses to hide its own body. Weapons stay out of the group: a gunner must still see the gun being aimed.
