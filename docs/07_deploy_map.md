...
- **M7: spawn points can be team-owned** (`owner_team`, 0 = contested). Enemy camps stay on the map, greyed and labelled `(enemy)`, for the same reason a full squadmate vehicle does: hiding an unavailable target makes it indistinguishable from one that does not exist. The three landmarks remain contested and open to everyone.
