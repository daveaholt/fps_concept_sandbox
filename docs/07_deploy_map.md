...
- **M7: the "enemies never" rule is amended for the minimap — firing reveals you.** An enemy who fires appears on the minimap as a transient ping at the shot's origin, fading after a few seconds. This is standard shooter behaviour, it rewards listening and punishes spraying, and it gives suppressors something to do if they are ever added (a `ProjectileParams` flag, not a new system).
  It is worth being precise about what this does and does not open up. The ping is a **position at a moment**, not a tracked entity: it never updates, so it says "someone fired here" rather than "an enemy is here now". A player who fires and moves is not followed. That is a different thing from rendering enemies on a map view, which stays forbidden — and the standing rule survives intact for everything else: **the minimap draws squadmates from `roster.squadmates()` and enemies only from gunshot events, never from the world.**
- **M7: you may deploy only at a point your own team owns, or onto a squadmate.** Contested landmarks are shown greyed and labelled `(contested)`, enemy camps `(enemy)`, closed ones `(closed)` — on the map but never selectable, for the same reason a full squadmate vehicle stays visible: hiding an unavailable target makes it indistinguishable from one that does not exist. Landmarks become spawnable the day there is a conquest mode to capture them with; the `owner_team` field is already the hook.
- **M7: the button's `disabled` state was set once at build time and never refreshed.** Markers are built when the map first opens, which can be before the roster has arrived and `my_team` is still `UNALIGNED` — so the enabled/disabled split was computed against the wrong team and then frozen. The server correctly refused the resulting deploy, which is how it surfaced: a marker that looked selectable, accepted a click, and then did nothing. `_refresh()` now recomputes `disabled` every frame from the same `available_to()` the server uses, and the suite asserts the two agree marker by marker.
