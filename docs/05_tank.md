...
Per 11's directional-sector spec, incoming damage is multiplied by impact location in hull-local space: **front ×1.0 · side ×1.0 · rear ×2.0 · top ×1.5** (all exported). Sector boundaries: ±45° cones fore/aft for front/rear, the rest is side; hits above deck height count as top. Vehicle damage landed at M7 (04), so these multipliers are live. The turret counts as top/side by the same local-space rule; no separate turret pool.
...
- **M7: `armour_side` strengthened 1.25 → 1.0 so the rear is the exception.** With the shell at 245 (floor 0.92) against a 500 hull, any side multiplier above ~1.02 dies in two — so making the flank survive three shells means giving it front-grade protection, not merely better protection. Side and front are therefore identical by arithmetic rather than by choice; there is no value between them that yields a different hit count.
- M7: `armour_top` deliberately stays at 1.5. It is the air-attack facing, and flattening it would take the helicopter's rockets from 5 hits on a tank from above to 8, which is the matchup the owner had already signed off. The rear being the only *ground* facing that dies in two is the intent; the roof is a separate axis.
- M7: strengthening the side also means rockets need **8** hits into a flank instead of 6, since the multiplier applies to every weapon. Unavoidable with one multiplier per sector, and noted here so it is not mistaken for a rocket change.
- M7: `verify_m5` pinned the sector multipliers as literals (`{"side": 1.25, …}`) and failed the moment the armour was legitimately tuned — the same landmine as the two recorded above. It now reads the expected values from the tank's own exports, so it still proves that a hit from direction X resolves to sector X without pinning what X is worth.
- **M7: small arms could destroy a tank, and fast — ten rifle rounds to the rear.** The armour sectors multiply *all* incoming damage, with no notion of penetration, so a rifle round was treated as a small shell: 25 damage doubled to 50 by the rear multiplier, and a single 30-round magazine did 1500 against a 500 hull. A minigun did 504 in one second. Reported from play as "I think my tank died to infantry", which it had.
  `small_arms_scale` (0.1) now multiplies the sector value for any round **not** flagged `explosive`, so shells and rockets are untouched and bullets are what they should be against armour: a full magazine into the softest facing does 150 of 500, and ten seconds of sustained minigun does about the same. Not nothing — a crippled tank can still be finished — but no longer a way to kill one.
- M7: the tank resists small arms; the **helicopter deliberately does not**. An airframe is not armour, and 14 rifle rounds still bring one down. The two vehicles want different rules and now have them.
