...
| `base_damage` | 25 | 20 | **245** direct + 4 m radius splash | Splash is server overlap query at impact; shell floor is 0.92, not 0.3 |
...
- **M7: hit confirmation.** `BallisticsManager` emits `hit_confirmed(shooter_peer, damage, killed)` on every damaging impact and every splash tick; `GameServer` relays it to that peer alone, and the HUD flashes a marker around the reticle — white for a hit, red and wider for a kill. It is deliberately server-sent rather than predicted on the shooter: the client does not know whether the round connected after rewind, and a marker that lies is worse than one that arrives 50 ms late.
- M7: the kill flag comes from sampling `is_alive()` either side of `apply_damage`, not from comparing health to zero, so it stays correct for targets that die on some other rule. A single burst can flash hit repeatedly and then kill once; the marker latches the kill colour for the rest of its fade so the last frame you see is the kill.
- **M7: the tank shell was retuned from 80 to 400 direct, with `damage_floor` raised from the default 0.3 to 0.9.** At 80 it took five hits to down a helicopter, which read as the helicopter being absurdly tough rather than the shell being weak. Raising `base_damage` alone would not have fixed it: energy damage scales with `(v / muzzle)²`, and with the shell's drag it falls below 90% of muzzle within about 20 m, so a 400-damage shell would still have failed to kill beyond point-blank. The floor is the right knob for a shell specifically — an explosive warhead's lethality does not depend on residual velocity the way a bullet's does. Bullets keep the 0.3 default.
- M7 resulting matrix, all ranges: helicopter (350 hull, flat ×1.0) dies to one shell; tank (500) survives one through the front or side and dies to one through the rear (×2.0) or top (×1.5); infantry die to a direct hit, which they previously survived at 80.
- **M7: `verify_m5` asserted `base_damage == 80.0` and broke the moment the value was legitimately tuned.** Replaced with the properties that actually matter — one-shots a helicopter at any range, does not one-shot a tank frontally — which is the same lesson recorded against the suspension and camera tunables. A test that pins a tuning constant is a test that has to be edited every time the game is tuned, and gets edited without thought.
- **M7: `ProjectileParams.explosive` and weapon-aware armour.** `resolve_sector()` now takes the round's params, so a target can answer differently per weapon class. It exists because the requested duel numbers are unsatisfiable with one multiplier per target: a shell that one-shots a 350-hull helicopter necessarily kills a 500-hull tank in two, so the helicopter has to be *specifically* soft to explosives rather than soft in general. It answers ×1.6 to anything flagged `explosive` and ×1.0 to everything else, which keeps small arms exactly where they were — a rifle still needs 14 rounds, a minigun 25.
- M7 duel matrix, holding at every range: tank front **3** shells, side/top/rear **2**; helicopter **1** shell or **4** rockets; tank front **8** rockets, unchanged. Derived rather than eyeballed — `verify_duel_math` asserts each count from both ends (N−1 hits at *maximum* damage must not kill, N hits at the *damage floor* must), so a count cannot silently drift into "sometimes 3, sometimes 4" as range changes.
- M7: the rocket's `damage_floor` went 0.6 → 0.9 for the same reason the shell's did. Without it, four rockets at the far end of their flight could not reach 350 while three fresh ones stayed under it — the window was empty. Rockets are short-ranged anyway, so close-quarters behaviour is unchanged.
- **M7: `explosive` now carries a second meaning — it is the closest thing the game has to a penetration model.** It began as "which rounds does a helicopter's thin skin resist less" and is now also "which rounds can hurt armour at all". That is two jobs for one boolean and it will eventually want to be a calibre or penetration value, but with four weapon types the flag is honest and there is nothing it currently gets wrong. Worth revisiting the day a weapon is neither clearly a bullet nor clearly a warhead.
