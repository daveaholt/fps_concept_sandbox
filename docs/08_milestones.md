...
- **Ammo: magazine, reserve and reload.** Raised by 14's HUD spec and promoted ahead of it. `WeaponDef` has no capacity and `InfantryState` has no ammo, so weapons are infinite today. It lands inside `InfantrySim.simulate`, so it carries the same replay-safety constraint as crouch, and it needs a reload binding in 02.
