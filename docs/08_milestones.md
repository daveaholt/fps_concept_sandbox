...
- **Death cam / kill cam.** Death currently cuts to the deploy screen on the same frame, which reads as abrupt — most shooters hold on the wreckage for a beat first. Four things stand in the way, and none are hard but none are free either:
  1. **There is nothing to look at.** `_release_peer` calls `queue_free()` on the body the instant it dies, so the corpse is gone before any camera could show it. Bodies would need to linger (and stop being ballistics targets while they do).
  2. **Wrecks despawn instantly too** — M7's destruction hides the hull immediately so that death is legible. That now works directly against a death cam, and the two want reconciling: probably wreck visible for the hold, then despawn.
  3. **No camera owns the moment.** `set_my_entity(null)` leaves no current camera until the deploy map's top-down one takes over. A death cam needs its own node placed at the death site.
  4. **A *kill* cam additionally needs the killer**, which the client is never told. `hit_confirmed` flows to the shooter; nothing flows to the victim. That is a small server addition but it is an addition.
  `RESPAWN_DELAY = 2.0` already exists in `GameServer` and is **read by nothing** — the intent was there and never wired, which is roughly the hold this wants.
