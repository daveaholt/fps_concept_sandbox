...
- **Squad / team management.** Teams, squad assignment, squad size **4** — two full squads per team at 8 a side, no remainder. This is a **blocker, not a peer**, for two things already on this list: the squad/team deploy-map markers (07 defers them explicitly until a team concept exists) and the minimap. Worth doing before either.
