...
- ~~**Ammo: magazine, reserve and reload.**~~ **Done.** Per-slot magazine and reserve in `InfantryState`, all transitions in the replay-safe step function, `reload` on R / pad X. See 03.
- ~~**ADS / zoom for all weapons.**~~ **Done.** Magnification and a movement penalty, no accuracy change — there is no spread in the game to reduce. Per-weapon `ads_fov`, sensitivity scaled to match. See 03. **Deterministic per-weapon spread is the follow-up** if ADS should ever confer accuracy: it must be replay-safe, so it needs a PRNG seeded from the command tick and shot index.
...
- ~~**Vehicle impact damage.**~~ **Done.** Velocity change per tick, squared past a per-vehicle tolerance — see 04. 06's 4 m/s turned out to be the wrong threshold to reuse: it is the clean-landing criterion, and a 1.2 m drop already exceeds it.
- **Minimap.** **Specced — see `14_hud.md`.** Drawn rather than rendered, so it can only show what it is handed. Ships as part of the HUD pass, since it owns a whole corner and laying the HUD out without it means laying it out twice. 07's "enemies never" rule is amended: firing reveals you, as a transient ping.
...
- ~~**AI bots.**~~ **Infantry bots done — see `15_bots.md`.** Server-side entities with no peer, navmesh pathing, nearest-target engagement, filling empty slots from a host toggle. **Bots using vehicles is still open** and staged deliberately.
- **HUD layout pass.** **Specced — see `14_hud.md`.** Blocked on the ammo system, which is being built first so the ammo readout is real rather than a placeholder. Also wants player names and per-player kills/deaths for the squad list and the after-battle report.
- ~~**Deploy screen on a controller.**~~ **Done.** Stick-driven directional selection with a bracket reticle; 07's flow section and 02 both updated.
- ~~**Death cam.**~~ **Done.** A 2.5 s hold on the death site before the deploy screen, orbiting slowly. All four blockers had to be cleared: bodies now linger instead of being freed on the frame they die, wrecks stay visible for the hold before despawning, `DeathCam` owns the camera in between, and `on_killed` carries the death position. **The kill cam half is still open** — framing the killer needs their identity sent to the victim, which nothing does today.
- **Optics and scope occlusion.** Extending ADS: distinct optics rather than one field of view per weapon, and a masked view — scope body, vignette, black surround — once magnification is high enough that a full-screen zoom reads as a cheat rather than a sight.
  What already supports it: `ads_fov` is per-weapon rather than global, `ZoomOptics.magnification()` exists, and `AimZoom` picks the target field of view from context, so a third source is a new branch rather than a new system. What it needs: an optic as its own resource so one weapon can take several, and a full-screen overlay in the HUD keyed on magnification — which is also the natural moment to hide the centre reticle, since a scoped sight should aim through its own reticle rather than the HUD's.
- **An infantry anti-tank weapon.** Now that small arms cannot meaningfully hurt a tank (05), infantry have no answer to armour at all — the counters are another tank, a helicopter's rockets, or nothing. A launcher is the obvious fix and would use the existing `explosive` flag and `BallisticsManager` with no new systems. Worth doing before any balance judgement about whether tanks are too strong, because right now they are unopposed rather than strong.
