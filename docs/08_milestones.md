...
- **Squad / team management.** Teams, squad assignment, squad size cap of 5. This is a **blocker, not a peer**, for two things already on this list: the squad/team deploy-map markers (07 defers them explicitly until a team concept exists) and the minimap. Worth doing before either. Note 8 a side with squads of 5 gives two uneven squads per team, so nothing should assume they are full or equal.
