...
- **Squad / team management — specced in `13_teams.md`, in progress.** Teams, squad assignment, squad size **4** — two full squads per team at 8 a side, no remainder. Scope settled with the owner: main menu + pre-game slot lobby, friendly fire off, squad-spawn, randomised spawn dispersal, shared (not team-owned) spawn points, and a `fill_with_bots()` seam that stays a no-op until AI bots land. This is a **blocker, not a peer**, for two things already on this list: the squad/team deploy-map markers (07 defers them explicitly until a team concept exists) and the minimap. Worth doing before either.
