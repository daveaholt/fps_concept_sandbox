...
- **M7: both menu routes stay available because there is no discovery.** A process cannot know another instance is already hosting — there is no broadcast, no lobby list, only a direct address — so HOST and JOIN are both offered and the answer comes from trying. What *was* wrong is that the two failures were handled asymmetrically: a port-bind failure is immediate, so HOST reported it and stayed put, while JOIN dismissed the menu on the press and only failed up to `CONNECT_TIMEOUT_SEC` (8 s) later, by which point nothing could bring the menu back. A wrong address left the player looking at the world with no menu, no lobby and no way forward.
  The menu now dismisses on `CONNECTED` rather than on the press, disables both buttons while an attempt is in flight, reports the failure, and calls `GameClient.abandon_connection()` so the torn-down peer does not block a retry.
