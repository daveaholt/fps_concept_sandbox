...
- ~~Heli under 100 ms interpolated control may simply feel bad despite mitigations — if so, the fallback experiment is client-authoritative heli *attitude* with server clamps.~~ **Resolved at M6: it flies acceptably under the shim and the loop is stable to 200 ms. Server authority holds for every vehicle; no fallback was needed.** See the M6 notes below and in 06.
...
- Post-M5: **a client driving the tank never saw its own shell, and this predates all the tracer tuning.** `GameClient.spawn_tracer` skipped the shot when `shooter_peer == get_peer_id()`, on the assumption that the shooter drew its own tracer through prediction. That holds for infantry — the predicted `player.gd` runs the sim, increments `shots_fired` and emits `fired` locally — but **not for vehicles**: `tank.gd._physics_process` returns immediately when it is not server-authoritative, so a client tank never fires locally and never emits anything. The gunner was skipped by the RPC *and* had no prediction to fall back on. The skip is now conditional on `is_predicting()`, which is already false for vehicles (a tank has no `is_predicted` method). On the listen-server host it always worked, because the server spawns into the same `BallisticsManager` that renders — which is exactly why it survived so long.
- Post-M5: the lesson for M6. **Every "the shooter is a special case" shortcut in the client assumes client-side prediction, and vehicles do not predict.** The helicopter will hit this same class of bug wherever the client short-circuits something because "we already did it locally". Worth grepping for `get_peer_id()` comparisons before wiring heli weapons.
- M6: **the flagged heli latency experiment has a measured answer: the control loop stays stable, and degrades gracefully.** A fixed automated pilot held a hover against a repeating gust while seeing only delayed state — same gains at every delay, so any change is the delay. RMS station-keeping error went **9.1 m at 0 ms → 9.7 m at 100 ms → 11.0 m at 200 ms** (1.2× across the whole range), with no divergence and no loss of control at any setting. There is no instability cliff between here and 200 ms, so the client-authoritative-attitude fallback in the open question above is **not needed on stability grounds**.
- M6: two honest limits on that result. It says the loop does not go unstable; it says **nothing about whether the delay feels good**, which is a pilot's call and the reason the M6 gate asks for a flown verdict as well. And it does not exercise `auto_level` at all — the assist only applies when the cyclic is centred, and the pilot is almost always holding some correction, which is why the assists-on and assists-off columns come out nearly identical. 06's claim that the assists carry the pilot through latency is therefore still unmeasured.
- M6: the rig only started producing meaningful numbers after a real bug was fixed. Every earlier run showed "lost control" at *every* delay including zero, which read like catastrophic latency sensitivity and was actually the helicopter flipping itself over under sustained stick (see 06). A control experiment on a vehicle with an unbounded failure mode measures the failure mode, not the control.
- **Post-M6: the 16-player target (8 v 8) doubles the design range, so three of this doc's assumptions need re-measuring rather than re-deciding.**
...
- **Post-M7: the menu broke vehicle replication entirely, and the symptom looked like a squad-spawn bug.** Entities cache `_server_authority` in `_ready` from `multiplayer.multiplayer_peer == null` — a deliberate choice, since asking per frame errors on a disconnected-but-existing peer. That was safe while `--client` created the peer before the level loaded. **The menu inverts it: the level loads with no peer, so every vehicle decides it is the authority, and then ignores every replicated state it is ever sent.** Infantry was unaffected because bodies are spawned after connecting, so their `_ready` sees a real peer.
  The reported symptom was "my squadmate's spawn marker vanishes when they board the tank", which sent me chasing the deploy map through three wrong fixes. The clue that solved it was a player noticing **the tank does not move on the kill-screen map** — vehicles were not replicating at all, and the missing seat data was one consequence of that, not a bug in its own right.
  Entities now expose `refresh_authority()` and the server calls it across the `controllable` group when hosting starts or a client connects.
- Post-M7: that is the third instance of the same shape — `ballistics.authoritative`, and now `_server_authority`. **Any value captured at level load is now captured before the menu has started networking.** When adding state that depends on the peer, either recompute it on connect or list it here.
- **Post-M7: the server dropped every passenger's input.** `_ingest` rejected any command whose sender was not the entity's `owner_peer`. That check was written when one peer owned one entity and it survived the seat rework unexamined, so a gunner's commands were discarded before reaching a seat queue — the minigun never fired, and switching seats silently killed your own input because `owner_peer` moved with the driver seat. Authority is now per **seat**: `_may_command()` accepts a peer that occupies any seat, and falls back to the `owner_peer` test for entities with no seats (infantry). The guard is still real — a peer with no seat is rejected.
- Post-M7: the reason `verify_weapons` passed while the gun was dead is that it called `push_command(cmd, seat)` on the entity directly, which is downstream of the rejection. **A test that injects at the entity cannot see a bug in the routing that feeds it.** Weapon tests now go through `submit_local_commands()`, the same entry point the client uses.
- **M7: gunshot positions are already broadcast to every client, and always have been.** `GameServer` sends `GameClient.spawn_tracer.rpc(origin, direction, params_id, shooter_peer)` unqualified on every shot from infantry, vehicle cannon and vehicle gun, because every client has to draw the tracer. That means the exact world position of every enemy shot is on the wire today, and a modified client could already plot them.
  This matters for the minimap's fire-reveal rule (07): showing an enemy ping when they fire **adds no exposure**, it surfaces something already sent. It also sharpens the accepted wallhack gap listed above — the honest statement is not "clients only know about what they can see" but "clients know every position that is replicated *and* every position anyone has fired from". Interest management, when it happens, has to cover tracer broadcasts and not just snapshots, or it will close the smaller hole and leave the larger one open.
