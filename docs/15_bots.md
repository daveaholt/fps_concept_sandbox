...
- **M7: bots spawn at a random point their own team may use.** The first version used `_default_spawn` for everyone, so fifteen bots landed on the same pad, were instantly inside each other's standoff, stopped moving and killed eleven of themselves in three and a half seconds. It looked like "bots do not move" until the corpse count explained it. Scattering them fixed the movement; **team-owned camps (13) fixed the slaughter**, which was the real complaint — with both sides spawning in the same places a bot-filled match ended in seconds.
