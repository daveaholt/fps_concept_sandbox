...
Destruction kills every occupant — one ticket each — empties the seats, blocks boarding, and queues the hull for revival after `VEHICLE_RESPAWN_SECONDS` (20 s) back at the transform it was recorded with in `_ready`. **An abandoned hull costs nobody a ticket**, since the cost is per occupant and there are none.
