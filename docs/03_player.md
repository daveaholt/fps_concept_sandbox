...
- Post-M3: rounds spawn at eye height along the aim vector, so from the shooter's own view tracers leave the eyeline rather than a weapon. Deferred to M7 with the viewmodel, and worth writing down *why* the obvious fix is wrong: simply spawning from the weapon proxy puts every round on a line parallel to the crosshair but offset ~24 cm to the right, so shots would miss by that much at every range. The correct fix is to keep the authoritative round on the eye-to-aim line and give the *tracer* a separate visual origin at the muzzle, converging on the aim point — which only makes sense once there is a viewmodel to converge from.

## Ammunition (M7)

`WeaponDef` carries `magazine_size`, `starting_reserve` and `reload_time`; rifle 30/150/2.1 s, pistol 12/60/1.5 s. `InfantryState` holds a magazine and a reserve **per weapon slot** plus one `reload_timer`, and every transition happens inside `InfantrySim.simulate` — it is sim state, not presentation, because the predicting client has to reach the same round count as the server or reconciliation fights it.

Rules: firing spends one round and is refused at zero. Pressing `reload` (R / pad B) starts a reload if the magazine is short and the reserve is not empty. **An empty magazine never reloads itself** — the player asks, the gun never decides. A reload moves `min(space in magazine, reserve)` across, so a last partial reload takes whatever is left. Switching weapons cancels a reload in progress — otherwise the timer finishes into the wrong gun.

- M7: `reload` shares pad **B** with `exit_vehicle`, which needs no arbitration: infantry cannot exit a vehicle they are not in, and a vehicle occupant is not running `InfantrySim`, so only one of the two can ever match. Keyboard keeps R for reload and F for exit.
- M7: auto-reload on an empty trigger pull was built and then removed on request. It hides the moment you run dry, and the moment you run dry is information — you should feel the click, not discover it two seconds later when the gun starts working again. The HUD prompts `RELOAD [B]` under the reticle instead, and `RELOADING` while it runs, which is placeholder for an animation.
- M7: ammo arrays are duplicated in `clone()` and compared in `equals_within()`. Both matter for reconciliation — a shallow copy would let a replayed step corrupt the prediction buffer's history, and a comparison that ignores ammo would let a mispredicted reload sit uncorrected forever.
- **M7: the `equals_within` change was silently a no-op at first** — the edit failed to match because the line it targeted ends in a `\` continuation, so the comparison shipped without ammo in it. Nothing broke, and nothing would have until a reload mispredicted under latency. It was caught only because a test asserted that two states differing *only* in ammo compare unequal. Worth generalising: when adding a field to a state struct, assert that the equality check actually notices it.

## Aiming down sights (M7)

Hold `zoom` (RMB / LT). The view narrows to the weapon's `ads_fov` — rifle 38°, pistol 55° against a 75° default, so the rifle is a touch under 2× — and movement drops to `ads_speed_scale` (0.45) of normal, sprint included. **Look sensitivity scales with the magnification**, so a given mouse or stick movement sweeps the same arc across the screen whether aimed or not; without that, zooming in makes aiming *harder*, which is the opposite of the point.

ADS **does not change accuracy**, because there is no inaccuracy to change: shots leave exactly along the aim vector and always have. That was the fork `08` flagged — it is magnification and a steadier stance, nothing more, and per-weapon spread is a separate piece of work if it is ever wanted.

- M7: the ADS flag rides `InputCommand` rather than living in `InfantryState`. The only sim effect is a speed multiplier read from the command each step, so there is no new state to clone, serialise or reconcile, and toggling it mid-run replays identically.
- M7: `GunnerZoom` became `AimZoom` and now serves infantry as well as vehicle seats — the vehicle case keeps its fixed 34°, infantry takes the weapon's. One controller, because "what is the current field of view" has exactly one answer and two of them would drift.
- **M7: the pure part lives in `ZoomOptics` for the third time running.** `AimZoom` reads `GameClient`, so it cannot be constructed in a `--script` harness, and the two sensitivity assertions silently did not run while the suite still reported zero failures — the same shape as `InteractionScanner`, `Minimap` and `Nameplates`, and this time the rule was already written down in `01`. The formula now lives somewhere with no autoload references.
