...
Rules: firing spends one round and is refused at zero. Pressing `reload` (R / pad X) starts a reload if the magazine is short and the reserve is not empty. Pulling the trigger on an empty magazine starts one for you. A reload moves `min(space in magazine, reserve)` across, so a last partial reload takes whatever is left. Switching weapons cancels a reload in progress — otherwise the timer finishes into the wrong gun.

- M7: `reload` shares pad **X** with `interact`. That is the console convention and it needs no arbitration here: near a vehicle the scanner takes the press and you board, and boarding makes the reload moot; away from one, `interact` matches nothing and the reload happens. The keyboard keeps them separate on R and E.
