...
- **Both corners are built in code, not in `hud.tscn`.** The scene keeps only the crosshair and the interaction prompt; everything else is assembled in `_ready` into two `VBoxContainer`s anchored to the bottom corners, growing up and inward. That is what makes the layout hold: containers compute their own size, so adding a readout pushes the stack into the screen instead of off it. The old `Bottom` panel is freed on load — it was the hand-positioned thing this doc exists to replace.
- **`Minimap` is fed, not wired.** It takes `show_world(origin, yaw, mates, spawns, pings)` once a frame and knows nothing about `GameClient`. The first version read the autoload directly and could not be constructed at all in a `--script` harness — autoload identifiers do not resolve at compile time there, the same trap that produced `InputFocus`. Passing the data in fixed the testability *and* made the code match what this doc already said: a widget fed explicit markers. It cannot leak an enemy position because it is never handed one.
- M7: `StatusBar` is one 40-line `Control` used for tickets, squad health and your own health. Three bars with three colours rather than three widgets.
- M7: callsigns are `Roster.callsign(slot)`, a static list of 16 names indexed by slot. No new replicated state — the slot is already known everywhere, so the name follows for free. Real names replace the lookup later without touching the HUD.
- M7: `verify_hud_fit` was rewritten rather than deleted. It exists because the HUD once fell off the bottom of the screen, and that invariant survives the redesign — it now asserts both corners stay inside a 1280x720 screen, are at least as tall as their contents, and **do not overlap each other**, which is the entire argument for two corners rather than one panel.
