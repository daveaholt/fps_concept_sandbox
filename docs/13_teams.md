...
- The deploy map **rebuilds its markers when the squadmate list changes**, not only when the screen opens. That was the actual reason a crewing squadmate still did not appear after the lookup was fixed: the marker list was built once on open, so a squadmate boarding a vehicle while you were already dead — or a moment after — never showed up. Three separate causes for one symptom, found one at a time.
- `verify_seat_marker` walks the whole client chain rather than any single link: nothing offered before deploy, offered on foot, still offered after boarding, resolved to the vehicle once the body is gone, rebuilt from a snapshot on a genuinely replicated copy, and the marker list refreshed on change. The earlier tests each proved one link and the chain was still broken.
- **M7: only the host can start the match.** Any peer holding a slot could start it, which made it a race — whoever pressed first decided when everyone else stopped picking. Starting is now rejected for any peer but the host, and the lobby hides the button from clients and tells them what they are waiting for instead of showing a control that would be refused. The host still needs a slot of their own, so a spectating host cannot start a match they are not in.
- **M7: players choose a callsign when they take a slot.** A `CALLSIGN` field sits above the slot grid; picking a slot sends whatever is in it. Blank falls back to the fixed per-slot name, so the field is optional and nothing is ever nameless. The name lives on the **slot**, not the peer, and is cleared when the slot is released — otherwise the next occupant would inherit a stranger's name.
- M7: names are sanitised **on the server**, not in the field: trimmed, capped at 14 characters, and stripped of control characters. The client's `max_length` is a convenience; the server treats the string as untrusted because it is drawn on every other player's screen.
- **M7: nameplates hang over players in the relationship colour** — green squadmate, blue team, red enemy, from `14`'s colour language. Friendly plates draw through walls; **enemy plates require line of sight and a shorter range** (70 m against 220 m). That asymmetry is the point: a plate you can see through a wall is a wallhack, and 10 already lists that gap as accepted rather than something to widen. The check is a world-mask raycast from the active camera, so cover works exactly as it looks.
- M7: the plate is a `Label3D` **outside** `Visual` on purpose. Possession moves everything under `Visual` to the own-body render layer, which every camera excludes — a plate parented there would have been hidden from everyone the moment its owner spawned.
- **M7: `owner_peer` was never replicated for infantry, so only the host could see nameplates.** Vehicles have carried `"o"` since M5, but a player body's snapshot had position, aim, health and weapon and nothing about *whose* it was — every remote body read `owner_peer` 0 on a client, which is not a peer, so no plate could resolve a name or a relationship. Players now ship `"o"` and `"t"` (team) alongside. Worth noting the symptom was silent for months: nothing needs the owner client-side until something wants to label a body.
- M7: a callsign can be changed after taking a slot, not only when picking one. `request_name` updates the slot in place and rebroadcasts; typing settles for 0.45 s before sending, so a rename costs one roster broadcast rather than one per keystroke. Enter sends immediately. A peer holding no slot is refused.
- **M7: empty slots can now be filled with bots** (15), on a host-only lobby toggle. The rule above was that you should not need a full roster to start, and that still holds — with the toggle off, empty slots stay empty exactly as before.
- **M7: team spawns exist after all, and the earlier "no team spawns" call is reversed.** With every point open to both sides, sixteen bots landed on top of each other, fought at contact range and burned a 25-ticket match in seconds. The fix is not to make the landmarks team-owned — that would take MainBase, Hilltop and the Airfield out of contention, and those are the places worth fighting over. Instead there are two new spawn points with **no map purpose at all**: `WestCamp` (team 1) and `EastCamp` (team 2), 140 m apart in empty corners, each with a tank and a helicopter parked beside it so neither side has to cross the map for armour.
  `SpawnPoint.owner_team` (0 = contested) drives all of it: the server refuses a deploy request at a point not open to your team, bots pick only from their own, and the deploy map greys enemy camps with an `(enemy)` label rather than hiding them — an absent marker and a forbidden one should not look the same.
- M7: a peer with no slot has no team, and therefore no spawn point at all. That is correct — everyone holds a slot before a match reaches PLAYING — but it means any harness that deploys someone now has to give them a slot first. Three suites had to be updated for exactly that.
- **M7: returning to the lobby reset the players and nothing else.** `_return_to_lobby()` released peers and cleared bots, but vehicles kept everything: a hull stayed damaged or wrecked where it had been abandoned, wreck and corpse timers kept running into the lobby so a hull could respawn mid-lobby, smoke kept pouring off a critical tank, and a vehicle still carried the previous match's `team`. The next match started with the last one's wreckage.
  `reset_for_lobby()` on each vehicle now empties the seats, drops ownership and team, revives the hull to full at its recorded spawn transform with zero velocity, clears gun heat, turret and gun angles, rotor and collective, and restores the rocket salvo. The server clears the wreck and corpse queues alongside. `verify_match_reset` makes a deliberate mess — damaged tank driven 30 m away with a hot gun and a slewed turret, helicopter wrecked in mid-air at full rotor — ends the match, and checks all of it.
